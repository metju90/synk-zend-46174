...
public class SmbTransport extends Transport implements SmbConstants {
    ...
    static synchronized SmbTransport getSmbTransport( UniAddress address, int port,
        ...
        synchronized( CONNECTIONS ) {
            ...
            if (cleoDebugOn) {
              System.err.println("getSmbTransport> calling new SmbTransport(" + address + ","
    ...
    private void negotiate( int port, ServerMessageBlock resp ) throws IOException {
        ...
        synchronized (sbuf) {
            ...
            if (port == 139) {
                ...
                in = socket.getInputStream();
                if (cleoDebugOn)
                  System.err.println(timeStr() + " SmbTransport.negotiate> LocalPort=" + socket.getLocalPort());
            }
    ...
    void checkStatus( ServerMessageBlock req, ServerMessageBlock resp ) throws SmbException {
        ...
        switch( resp.errorCode ) {
            ...
            case NtStatus.NT_STATUS_UNSUCCESSFUL:
                throw new SmbException( resp.errorCode, null, this );
            default:
                throw new SmbException( resp.errorCode, null );
        }
    ...
    void send( ServerMessageBlock request, ServerMessageBlock response ) throws SmbException {
        ...
        try {
                ...
                try {
                    ...
                    synchronized (this) {
                        ...
                        try {
                            ...
                            long endTimeMs;
                            resp.expiration = startTimeMs + timeout;
                            while( resp.hasMoreElements() ) {
                                wait( timeout );
                                endTimeMs = System.currentTimeMillis();
                                timeout = resp.expiration - endTimeMs;
                                if ((cleoDebugOn) && (timeout <= 0) && (!resp.hasMoreElements())) {
                                  System.err.println("-----------------------------------------------------------------------------------");
                                  System.err.println(timeStr() + " Thread=" + Thread.currentThread().getName() + "  " + this + " would have timed out previously");
                                  System.err.println("-----------------------------------------------------------------------------------");
                                }
                                if ((timeout <= 0) && (resp.hasMoreElements())) {
                                    if (cleoDebugOn) {
                                      System.err.println("...............................................................");
                                      System.err.println(timeStr() + " Thread=" + Thread.currentThread().getName() + "  " + this + " timedout after " + String.format("%.3f", (endTimeMs - startTimeMs) / 1000.0) +
                                                         " sec waiting for response to " + req );
                                      Thread.dumpStack();
                                      System.err.println("...............................................................");
                                    }
            ...
            throw new SmbException( ioe.getMessage(), ioe, this );
        }
